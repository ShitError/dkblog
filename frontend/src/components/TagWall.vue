<template>
  <div class="tagcloud-all"
       ref="tagcloudall">
    <a v-for="item of tagList" :key="item.id" :href=item.url :style="'color:' + item.color + ';top: 0;left: 0;filter:none;'">{{ item.name }}</a>
  </div>
</template>
      ...
      this.axios.get("/api/tags").then((res)=>{
        ...
        if(res.data.flag) {
          res.data.data.recordList.forEach((item,index) => {
            if(item){
              tagListOrg[index].color = "rgb(" + this.getRandomNum() + "," + this.getRandomNum() + "," + this.getRandomNum() + ")";
              tagListOrg[index].name = item.tagName;
              tagListOrg[index].url = "/tags/" + item.id;
            }
          })
        }
      }).catch(()=>{
