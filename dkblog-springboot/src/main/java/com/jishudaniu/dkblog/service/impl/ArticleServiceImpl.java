...
import com.jishudaniu.dkblog.dao.*;
import com.jishudaniu.dkblog.pojo.dto.*;
import com.jishudaniu.dkblog.pojo.entity.*;
import com.jishudaniu.dkblog.exception.BizException;
import com.jishudaniu.dkblog.service.ArticleService;
import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.jishudaniu.dkblog.service.ArticleTagService;
import com.jishudaniu.dkblog.service.RedisService;
import com.jishudaniu.dkblog.service.TagService;
import com.jishudaniu.dkblog.strategy.context.SearchStrategyContext;
import com.jishudaniu.dkblog.util.BeanCopyUtils;
import com.jishudaniu.dkblog.util.PageUtils;
import com.jishudaniu.dkblog.util.UserUtils;
...
import static com.jishudaniu.dkblog.constant.CommonConst.*;
import static com.jishudaniu.dkblog.constant.RedisPrefixConst.*;
import static com.jishudaniu.dkblog.enums.ArticleStatusEnum.DRAFT;
import static com.jishudaniu.dkblog.enums.ArticleStatusEnum.PUBLIC;
...
public class ArticleServiceImpl extends ServiceImpl<ArticleDao, Article> implements ArticleService {
    ...
    public List<ArticleHomeDTO> listArticles() {
        List<ArticleHomeDTO> articles = new ArrayList<>(articleDao.listArticles(PageUtils.getLimitCurrent(), PageUtils.getSize()));
        for (ArticleHomeDTO article : articles) {
            Double score = redisService.zScore(ARTICLE_VIEWS_COUNT, article.getId());
            if (Objects.nonNull(score)) {
                article.setViewsCount(score.intValue());
            }
//            article.setLikeCount((Integer) redisService.hGet(ARTICLE_LIKE_COUNT, article.getId().toString()));
            Integer commentCount = commentDao.selectCount(new LambdaQueryWrapper<Comment>()
                    .eq(Objects.nonNull(article.getId()), Comment::getArticleId, article.getId())
                    .isNull(Objects.isNull(article.getId()), Comment::getArticleId)
    ...
    @Override
    public List<ArticleRecommendDTO> listArticlesnews() {
        // 查询最新文章
        ...
        List<ArticleRecommendDTO> newestArticlelist = null;
        try {
            newestArticlelist = new ArrayList<>(newestArticleList.get());
    ...
    @Override
    public List<ArticleRankDTO> listArticleshots() {
        // 查询最热文章
